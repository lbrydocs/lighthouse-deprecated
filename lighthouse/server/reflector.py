...
class SDReflectorServer(Protocol):
    ...
    def connectionLost(self, reason=failure.Failure(error.ConnectionDone())):
        log.debug("Reflector upload from %s finished" % self.peer.host)
    ...
    def _on_completed_blob(self, blob, response_key):
        ...
        with blob.open_for_reading() as sd_file:
            sd_blob_data = sd_file.read()
        decoded_sd_blob = json.loads(sd_blob_data)
        size = sum([blob.get('length', 0) for blob in decoded_sd_blob['blobs']])
        yield self.storage.save_stream_size(b_h, size)
    ...
    def get_descriptor_response(self, sd_blob):
        if sd_blob.blob_hash not in list(self.cache.get_sd_hash().itervalues()):
            log.debug("Rejecting non sd blob")
            return defer.succeed({SEND_BLOB: False})
        if sd_blob.is_validated():
            log.debug("Already have blob")
            d = defer.succeed({SEND_SD_BLOB: False})
            d.addCallback(self.request_needed_blobs, sd_blob)
        else:
            log.debug("Requesting blob")
    ...
    def determine_missing_blobs(self, sd_blob):
        with sd_blob.open_for_reading() as sd_file:
            sd_blob_data = sd_file.read()
        decoded_sd_blob = json.loads(sd_blob_data)
        sd_hash = sd_blob.blob_hash
        return self.get_unvalidated_blobs_in_stream(decoded_sd_blob, sd_hash)

    @defer.inlineCallbacks
    def get_unvalidated_blobs_in_stream(self, sd_blob, sd_hash):
        size = sum([blob.get('length', 0) for blob in sd_blob['blobs']])
        yield self.storage.save_stream_size(sd_hash, size)
        needed = []
        for blob in sd_blob['blobs']:
            if 'blob_hash' in blob and 'length' in blob:
                blob_hash, blob_len = blob['blob_hash'], blob['length']
                blob = yield self.blob_manager.get_blob(blob_hash, blob_len)
    ...
    def get_blob_response(self, blob):
        if blob.blob_hash not in list(self.cache.get_sd_hash().itervalues()):
            log.debug("Rejecting non sd blob")
            return defer.succeed({SEND_BLOB: False})
...
class SDReflectorServerFactory(ServerFactory):
    protocol = SDReflectorServer

    def __init__(self, peer_manager, blob_manager, cache, storage):
