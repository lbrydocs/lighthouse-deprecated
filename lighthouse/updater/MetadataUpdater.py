...
class MetadataUpdater(object):
    ...
    def _load_stream_size(self, sd_hash):
        def _save(size):
            if size:
                log.info("Load size for %s", sd_hash)
    ...
    def _load_sd_attempts(self, sd_hash):
        def _save(attempts):
            if attempts:
                log.info("Loaded %i attempts for %s", attempts, sd_hash)
    ...
    def _load_claim(self, tx):
        ...
        def _save(claim_id, name, tx, n, encoded_meta):
            ...
            try:
                ...
                return self._notify_bad_metadata(txid)
    ...
    def _add_claims_for_height(self, height):
        ...
        if height % 100 == 0:
            ...
            self._last_time = time.time()
            log.info("Imported %i blocks, %f blocks/s", height, bps)
    ...
    def _notify_bad_metadata(self, txid):
        # log.info("claim tx %s does not conform to any specification", txid)
